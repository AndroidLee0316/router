...
public class BaseJumper {
    ...
    /*************以下为Activity 跳转的*****************/
    public static void jumpARouter(@NonNull String path) {
        jumpBundleARouter(path, null);
    }
     ...
     */
    public static void jumpBundleARouter(@NonNull String path, @Nullable Bundle data) {
        jumpBundleARouter(path, null, -1, data);
    }
     ...
     * @param data
     */
    public static void jumpBundleARouter(@NonNull String path, @Nullable Activity context, int requestCode, @Nullable Bundle data) {
        try {
            Postcard postcard = getPostcardFromPath(path);
            if (data != null)
                postcard.with(data);
     ...
     *                  2.2 ps  url="router://com.pingan.nt/login/activity/main?age=18&name=jay&job=android"
